...
contract('Bridge - [deposit - Generic]', async () => {
    ...
    beforeEach(async () => {
        await Promise.all([
            ...
            //BridgeInstance = BridgeContract.new().__Bridge_init(originChainID, [], 0, 0, 100).then(instance => BridgeInstance = instance)
        ]);

        BridgeInstance = await BridgeContract.new();
        await BridgeInstance.__Bridge_init(originChainID, [], 0, 0, 100);
